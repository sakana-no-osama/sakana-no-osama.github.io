...
TIMEOUT = 20
TARGET_YEAR = "2026"
IN_CSV = "match_sources_2026.csv"
OUT_CSV = "matches_2026.csv"
...
def read_sources(path: Path) -> List[SourceRow]:
    ...
    if not rows:
        raise HoldError("no 2026 match page rows found")
...
def find_match_start(compact: str) -> int:
    ...
    date_time_pat = re.compile(
        r"2026年\s*[0-9０-９]{1,2}月\s*[0-9０-９]{1,2}日"
    ...
    for m in date_time_pat.finditer(compact):
        ...
        if "JFA U-15女子サッカーリーグ 2026" in window:
...
def parse_section(block: str) -> str:
    m = re.search(r"JFA U-15女子サッカーリーグ 2026\s+関東[１２12１二一]+部\s*\|\s*第\s*([0-9０-９]+)\s*節", block)
    if not m:
